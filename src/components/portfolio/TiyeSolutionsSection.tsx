...
const services = [
  { icon: Search, label: 'SEO', color: 'from-red-500 to-red-600', bgColor: 'bg-red-500/10', textColor: 'text-red-500' },
  { icon: FileText, label: 'Content Creation', color: 'from-amber-500 to-orange-500', bgColor: 'bg-amber-500/10', textColor: 'text-amber-500' },
  { icon: Palette, label: 'UI/UX Design', color: 'from-purple-500 to-pink-500', bgColor: 'bg-purple-500/10', textColor: 'text-purple-500' },
  { icon: Code, label: 'Frontend Edits', color: 'from-emerald-500 to-teal-500', bgColor: 'bg-emerald-500/10', textColor: 'text-emerald-500' },
];
...
export const TiyeSolutionsSection = () => {
  ...
  const ImageGallery = ({ images, currentIndex, onNavigate }: { 
    images: string[]; 
    currentIndex: number; 
      ...
      <div className="space-y-4">
        ...
        <div className="relative group">
          ...
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-[#BE1522] text-white shadow-lg hover:scale-110 hover:shadow-[#BE1522]/50 transition-all duration-300"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button
            onClick={() => onNavigate('next')}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-[#BE1522] text-white shadow-lg hover:scale-110 hover:shadow-[#BE1522]/50 transition-all duration-300"
          >
        ...
        <div className="grid grid-cols-4 gap-2">
          {remainingImages.slice(0, 4).map((image, index) => {
            ...
            return (
              <button
                key={index}
                onClick={() => onNavigate(originalIndex > currentIndex ? 'next' : 'prev')}
  ...
  return (
    <section className="bg-gradient-to-br from-card/50 via-card/30 to-[#BE1522]/5 backdrop-blur-lg rounded-3xl border border-[#BE1522]/20 p-8 md:p-12 relative overflow-hidden">
      {/* Colorful geometric pattern background */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none">
        <div className="absolute inset-0 light-subtle-pattern"></div>
      </div>
      <div className="absolute top-0 right-0 w-64 h-64 bg-[#BE1522]/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              <h4 className="text-lg md:text-xl font-semibold text-foreground flex items-center gap-2">
                <span className="w-8 h-1 bg-[#BE1522] rounded-full"></span>
                About the Project
              </h4>
            ...
            <div className="space-y-4">
              ...
              <ul className="grid grid-cols-2 gap-3">
                {services.map((service, index) => (
                  <li 
                    key={index}
                    className={`flex items-center gap-3 p-3 rounded-xl ${service.bgColor} border border-border/50 hover:scale-105 transition-all duration-300 group cursor-default`}
                  >
            ...
            <Button 
              asChild 
              className="w-full sm:w-auto bg-[#BE1522] hover:bg-[#BE1522]/90 text-white shadow-lg shadow-[#BE1522]/25 hover:shadow-[#BE1522]/50 hover:scale-105 transition-all duration-300"
            >
          ...
          <div className="space-y-4">
            <Tabs defaultValue="big-service" className="w-full">
              <TabsList className="w-full grid grid-cols-2 bg-background/50 border border-[#BE1522]/20 rounded-xl p-1">
                <TabsTrigger 
                  value="big-service" 
                  className="rounded-lg data-[state=active]:bg-[#BE1522] data-[state=active]:text-white data-[state=active]:shadow-lg"
                >
                  Big Service Page
                </TabsTrigger>
                <TabsTrigger 
